...
#### Usage:
```bash
./host.py list              # List all entries
./host.py add <ip> <domain> # Add a new entry
./host.py replace <new_ip> <domain> # Replace an existing entry's IP
./host.py rm                # Delete an entry or specific domain
```
